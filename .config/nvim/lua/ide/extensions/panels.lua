...
function Panels:on_register(ctx)
    ctx:command('IDEStatus', function()
        IDE.toolkit.InfoPanel.ide_status():show()
    end, { desc = 'Show IDE status panel' })

    ctx:command('IDELsp', function()
    ...
    ctx:command('IDEGit', function()
        ...
        IDE.toolkit.InfoPanel({
            title = '  Git',
            sections = sections,
        }):show()
    end, { desc = 'Show git status' })

    ctx:command('IDEExtensions', function()
        ...
        SelectPicker({
            ...
            on_select = function(item)
                ...
                local status = ext:is_errored() and 'ERRORED' or (ext:is_enabled() and 'Enabled' or 'Disabled')
                local msg = ext:name() .. '\n'
                    .. 'Status: ' .. status .. '\n'
