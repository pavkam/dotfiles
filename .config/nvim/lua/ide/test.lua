...
function M.run(filter)
    ...
    suite('Notify', function()
        test('info records to history', function()
            local count_before = #IDE.ui._notify:history()
            IDE.ui._notify:info('test message')
            local count_after = #IDE.ui._notify:history()
            assert_eq(count_after, count_before + 1)
            assert_eq(IDE.ui._notify:history()[1].message, 'test message')
        end)
        test('history is capped at 100', function()
            local n = IDE.ui._notify
            assert_true(#n:history() <= 100)
        end)
        test('progress returns a handle', function()
            local h = IDE.ui._notify:progress('loading...')
            assert_not_nil(h)
            assert_not_nil(h.update)
            assert_not_nil(h.finish)
        end)
        test('progress handle can update', function()
            local h = IDE.ui._notify:progress('step 1')
            h:update('step 2', 0.5)
            assert_eq(h._msg, 'step 2')
            assert_eq(h._pct, 0.5)
            assert_false(h._done)
        end)
        test('progress handle can finish', function()
            local h = IDE.ui._notify:progress('working')
            h:finish('done!')
            assert_true(h._done)
            assert_eq(h._pct, 1)
        end)
        test('progress finish is idempotent', function()
            local h = IDE.ui._notify:progress('task')
            h:finish('complete')
            h:finish('again')
            assert_true(h._done)
        end)
        test('progress update after finish is a no-op', function()
            local h = IDE.ui._notify:progress('task')
            h:finish()
            h:update('should not change')
            assert_eq(h._msg, 'task')
        end)
    end)
    ...
    suite('IDE.ui behavior', function()
        ...
        test('finder has all search methods', function()
            local f = IDE.ui.finder
            assert_not_nil(f)
            assert_type(f.files, 'function')
            assert_type(f.grep, 'function')
            assert_type(f.symbols, 'function')
            assert_type(f.workspace_symbols, 'function')
            assert_type(f.buffers, 'function')
            assert_type(f.recent, 'function')
            assert_type(f.references, 'function')
            assert_type(f.definitions, 'function')
            assert_type(f.implementations, 'function')
            assert_type(f.type_definitions, 'function')
            assert_type(f.keymaps, 'function')
            assert_type(f.diagnostics, 'function')
        end)
        test('finder symbol icons table is populated', function()
            -- The Finder module should have symbol icons for LSP kinds
            local Finder = require 'ide.Finder'
            assert_not_nil(Finder)
        end)
    end)
    ...
    -- Verify that critical extensions loaded and wired up
    -- ═══════════════════════════════════════════════════════

    suite('Extension: EditingKeymaps', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('EditingKeymaps'))
        end)
        test('undo keymap is set', function()
            local maps = vim.api.nvim_get_keymap('n')
            local found = vim.iter(maps):any(function(m) return m.lhs == '<C-Z>' or m.desc == 'Undo' end)
            assert_true(found, 'Ctrl+Z or Undo keymap must exist')
        end)
        test('comment keymap is set (gcc)', function()
            local maps = vim.api.nvim_get_keymap('n')
            local found = vim.iter(maps):any(function(m) return m.lhs == 'gc' or m.lhs == 'gcc' end)
            assert_true(found, 'gc or gcc keymap must exist')
        end)
    end)

    suite('Extension: BufferKeymaps', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('BufferKeymaps'))
        end)
        test('file.save action exists', function()
            assert_true(IDE.actions:has('file.save'))
        end)
        test('file.open action exists', function()
            assert_true(IDE.actions:has('file.open'))
        end)
        test('editor.close action exists', function()
            assert_true(IDE.actions:has('editor.close'))
        end)
    end)

    suite('Extension: FileOperations', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('FileOperations'))
        end)
        test('file.new action exists', function()
            assert_true(IDE.actions:has('file.new'))
        end)
        test('file.rename action exists', function()
            assert_true(IDE.actions:has('file.rename'))
        end)
    end)

    suite('Extension: WindowChrome', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('WindowChrome'))
        end)
        test('has a frame', function()
            local ext = IDE:extension('WindowChrome')
            assert_not_nil(ext._frame)
        end)
    end)

    suite('Extension: Completion', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Completion'))
        end)
        test('editor.completion action exists', function()
    ...
    suite('Extension: Terminal', function()
        ...
        test('terminal.toggle action exists', function()
            assert_true(IDE.actions:has('terminal.toggle'))
        end)
        test('IDE.terminal is accessible', function()
            assert_not_nil(IDE.terminal)
        end)
    end)

    suite('Extension: FindReplace', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('FindReplace'))
        end)
        test('editor.findReplace action exists', function()
            assert_true(IDE.actions:has('editor.findReplace'))
        end)
    end)

    suite('Extension: Folding', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Folding'))
        end)
    end)

    suite('Extension: FeatureToggles', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('FeatureToggles'))
        end)
        test('auto_formatting toggle exists', function()
            assert_not_nil(IDE.config:is_enabled('auto_formatting'))
        end)
    end)

    suite('Extension: FormatOnSave', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('FormatOnSave'))
        end)
    end)

    suite('Extension: LintOnChange', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('LintOnChange'))
        end)
    end)

    suite('Extension: Desktop', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Desktop'))
        end)
    end)

    suite('Extension: Panels', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Panels'))
        end)
        test('view.ideStatus action exists', function()
            assert_true(IDE.actions:has('view.ideStatus'))
        end)
        test('view.lspStatus action exists', function()
            assert_true(IDE.actions:has('view.lspStatus'))
        end)
        test('view.gitStatus action exists', function()
            assert_true(IDE.actions:has('view.gitStatus'))
        end)
    ...
    suite('Extension: Outline', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Outline'))
        end)
        test('view.outline action exists', function()
            assert_true(IDE.actions:has('view.outline'))
        end)
    end)

    suite('Extension: SessionPersistence', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('SessionPersistence'))
        end)
    end)

    suite('Extension: TestRunner', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('TestRunner'))
        end)
        test('IDETest command exists', function()
            local cmds = vim.api.nvim_get_commands({})
            assert_not_nil(cmds['IDETest'])
        end)
        test('test keymaps are set', function()
            local ext = IDE:extension('TestRunner')
            assert_true(#(ext._keymaps or {}) >= 4, 'should have at least 4 test keymaps')
        end)
    end)

    suite('Extension: Spelling', function()
        test('extension is registered', function()
            assert_not_nil(IDE:extension('Spelling'))
        end)
    end)
