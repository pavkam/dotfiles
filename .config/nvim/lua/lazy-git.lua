...
---@type string
local custom_config_path = vim.fs.normalize(vim.fs.joinpath(ide.fs.CACHE_DIRECTORY, 'lazy-git-config.json'))
---@type string|nil
local config_dir
...
local function set_ansi_color(color_key, color)
    xassert {
        color_key = { color_key, 'integer' },
        color = { color, { 'string', ['*'] = '^#%x%x%x%x%x%x$' } },
    }
...
local function with_updated_color_theme(done_fn)
    ...
    local function apply()
        ...
        local ok, err = ide.fs.write_text_file(custom_config_path, json)
        if not ok then
            ide.tui.error(
                string.format(
...
if vim.fn.executable(command) == 1 then
    vim.fn.user_command('Lazygit', function()
        run_lazygit()
    end, { desc = 'Run Lazygit', nargs = 0 })

    require('keys').map('n', '<leader>g', function()
        vim.cmd 'Lazygit'
    end, { icon = require('icons').UI.Git, desc = 'Lazygit' })
end
